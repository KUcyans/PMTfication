...
conditions=(
    ...
    # "Corsika 0003000-0003999 1 9 20000"   # 1-9
...
for condition in "${conditions[@]}"; do
    ...
    for PART in $(seq $START_PART $END_PART); do
        ...
        echo "[$timestamp] Submitting job for $SnowstormOrCorsikaValue $SUBDIR part $PART with $NEVENTS_VALUE events per shard (SECOND_ROUND=$SECOND_ROUND)"
        
        sbatch --export=SnowstormOrCorsika=$SnowstormOrCorsikaValue,SUBDIR=$SUBDIR,PART=$PART,NEVENTS=$NEVENTS_VALUE,SECOND_ROUND=$SECOND_ROUND PMTfy_by_part.sh
    done
done
