...
class IntraTravelDistanceFilter(EventFilter):
    ...
    def _set_valid_event_nos(self): 
        ...
        required_columns = {"post_vertex_intraIceCube_segment", "N_doms", "event_no"}
        ...
        intra_icecube_lepton_travel_distance = truth_table.column("post_vertex_intraIceCube_segment").to_numpy()
        valid_indices = np.where(intra_icecube_lepton_travel_distance > self.min_travel_distance)[0]
        if len(valid_indices) == 0:
            self.logger.warning(f"No valid events found with intra-ICECUBE line segment > {self.min_travel_distance} in {self.subdir_no}/{self.part_no}. Skipping filtering.")
            return
        self.valid_event_nos = set(truth_table.take(valid_indices).column("event_no").to_pylist())
        self.logger.info(f"Extracted {len(self.valid_event_nos)} valid events with post_vertex_intraIceCube_segment > {self.min_travel_distance}.")